...
class BOT():
    ...
    def self_play_train(self, args):
        ...
        def gen_data():
            ...
            return [(x[0], x[1]) for x in history if (game_result == 0 or x[2] == game_result)], game_result

        data = []
        for i in range(args['num_of_generate_data_for_train']):
            if args['verbose']:
                print('Self playing', i + 1)
            game_data, game_result = gen_data()
            data += game_data
            if game_result == 1:
                win_count += 1
            elif game_result == -1:
        ...
        win_percentage = (win_count / args['num_of_generate_data_for_train']) * 100
        lose_percentage = (lose_count / args['num_of_generate_data_for_train']) * 100
        draw_percentage = (draw_count / args['num_of_generate_data_for_train']) * 100
